...
void eval_micro_sequencer() {
   ...
   */
	...
	marmux_data = alu_data = shf_data = mdr_data = BUS = 0;
	NEXT_LATCHES = CURRENT_LATCHES;
	...
	if (IRD == 1) {
		...
		int COND_IR11 = (COND & 0x1) && ((COND >> 1) & 0x1) && ((CURRENT_LATCHES.IR >> 11) & 0x1);
		int COND_R = (COND & 0x1) && !((COND >> 1) & 0x1) && (CURRENT_LATCHES.READY);
		int COND_BEN = !(COND & 0x1) && ((COND >> 1) & 0x1) && (CURRENT_LATCHES.BEN);
		int COND_J = (COND_IR11) + (COND_R << 1) + (COND_BEN << 2);
		next = J | COND_J;
	}
...
void eval_bus_drivers() {
   ...
   *		 Gate_MDR.
   */    
	/*figure out the values that eventually get written on the bus*/
...
void drive_bus() {
   ...
   * tristate drivers. 
   */
   /*set the bus equal to whatever it needs to be*/  
...
void latch_datapath_values() {
   ...
   * after drive_bus.
   */       
	/*load everything (e.g. using LDMAR and whatnot)*/
